/**
* Copyright (C), 2016-2020, UNIONMAN Tech. Co., Ltd.
* All rights reserved.
*
* @file        hi_list.h
...
* @date      2016.06.06
*/
#ifndef __HI_LIHI_H__
#define __HI_LIHI_H__
...
/**init list head node*/
#define HI_LIHI_INIT_HEAD_DEFINE(name) LIST_HEAD_INIT(name)

/**list head node define*/
#define HI_LIHI_HEAD_DEFINE(name) LIST_HEAD(name)

/**init list head node pointer*/
#define HI_LIHI_INIT_HEAD_PTR(ptr) INIT_LIST_HEAD(ptr)
 ...
 */
#define HI_LIHI_ENTRY(ptr, type, member) cbb_list_entry(ptr, type, member)
...
/**
 *  iterate over a list from tail
 * @pos:   input, output, the &struct HI_List_Head_S to use as a loop counter.
 * @head:  input, the head of the list.
 */
#define HI_List_For_Each_Tail(pos, head) cbb_list_for_each_tail(pos, head)

/**
 * iterate over a list from tail safe against removal of list entry
 * @pos:   input, output,  the &struct HI_List_Head_S to use as a loop counter.
 * @n:     input, another &struct HI_List_Head_S to use as temporary storage
 * @head:  input, the head of the list.
 */
#define HI_List_For_Each_Safe_Tail(pos, n, head) cbb_list_for_each_safe_tail(pos, n, head)
